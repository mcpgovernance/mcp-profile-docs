...
- Taxon scientific name + authority [mandatory for each taxon to be cited] - examples: 
...
Instead, it seems preferable to hold this domain knowledge in one of the chosen schemas (DwC) and embed that schema into the MCP. The advantages of this approach are:
...
Two examples are presented here. The first shows how taxonomic information that uses DwC can be embedded in the MCP record. The second shows how DwC taxonomic information could be XLink'd from an external vocabulary service.
